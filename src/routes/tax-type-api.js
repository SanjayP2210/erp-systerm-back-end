const express = require('express');
const router = express.Router();
const TaxType = require('../models/TaxType');


// Create a new TaxType
router.post('/', async (req, res) => {
  try {
    console.log('req.body', req.body);
    const todo = new TaxType({ ...req.body });
    await todo.save();
    res.status(200).json(todo);
  } catch (error) {
    res.status(400).json({ error: 'Could not create a new todo' });
  }
});

// Get all TaxType
router.get('/', async (req, res) => {
  const purchaseOrder = await TaxType.find();
  if (purchaseOrder) {
    res.status(200).json(purchaseOrder);
  } else {
    res.status(400).send('data not found');
  }
});

// Update a TaxType
router.put('/:id', async (req, res) => {
  try {
    const todo = await TaxType.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!todo) {
      return res.status(404).json({ error: 'TaxType not found' });
    }
    res.status(200).json(todo);
  } catch (error) {
    res.status(400).json({ error: 'Could not update the todo' });
  }
});

// Delete a TaxType
router.delete('/:id', async (req, res) => {
  try {
    const todo = await TaxType.findByIdAndDelete(req.params.id);
    if (!todo) {
      return res.status(404).json({ error: 'TaxType not found' });
    }
    res.status(200).send('data deleted succesfully');
  } catch (error) {
    res.status(400).json({ error: 'Could not delete the todo' });
  }
});

module.exports = router;
